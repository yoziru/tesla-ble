...
TEST_F(ClientTest, SetConnectionID) { EXPECT_NO_THROW(client->setConnectionID(TestConstants::TEST_CONNECTION_ID)); }
...
TEST_F(ClientTest, BuildWhiteListMessageInvalidRole) {
  ...
  int result = client->buildWhiteListMessage(static_cast<Keys_Role>(999), VCSEC_KeyFormFactor_KEY_FORM_FACTOR_CLOUD_KEY,
                                             whitelist_message_buffer, &whitelist_message_length);
