...
TEST_F(ProtocolEdgeCasesTest, InvalidPublicKeyFormat) {
    ...
    int result = peer.loadTeslaKey(bad_pubkey, 65);
...
TEST_F(ProtocolEdgeCasesTest, CounterOverflow) {
    Peer peer(UniversalMessage_Domain_DOMAIN_INFOTAINMENT, crypto_context, TestConstants::TEST_VIN);
    peer.setCounter(0xFFFFFFFF);
    peer.incrementCounter();
    EXPECT_EQ(peer.getCounter(), 0) << "Counter should wrap to 0 on overflow";
}
...
TEST_F(ProtocolEdgeCasesTest, MalformedMetadata) {
    ...
    EXPECT_EQ(ret, 0);
    ret = mbedtls_gcm_crypt_and_tag(&gcm, MBEDTLS_GCM_ENCRYPT, 4, ad_hash, 12, nullptr, 0, plaintext, ciphertext, 16, tag);
...
TEST_F(ProtocolEdgeCasesTest, RequestHashInvalidTagLength) {
    ...
    uint8_t hash[33];
    size_t hash_length;
    int result = peer.constructRequestHash(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED, short_tag, 4, hash, &hash_length);
    ...
    EXPECT_EQ(hash[0], static_cast<uint8_t>(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED)) << "First byte should be auth type";
}
...
TEST_F(ProtocolEdgeCasesTest, HmacAllZeroKey) {
    ...
    bool all_zero = true;
    for (int i = 0; i < 32; ++i) if (hmac[i] != 0) all_zero = false;
