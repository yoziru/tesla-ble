...
protected:
    void SetUp() override {
        ...
        test_epoch = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
...
TEST_F(ADBufferConstructionTest, RequestMetadataFormat) {
    ...
    // Test request format
    int result = vcsec_peer->constructADBuffer(
...
TEST_F(ADBufferConstructionTest, FlagsConditionalInclusion) {
    ...
    int result1 = vcsec_peer->constructADBuffer(
        Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
        test_vin.c_str(),
        expires_at,
        ad_buffer_no_flags,
    ...
    int result2 = vcsec_peer->constructADBuffer(
        Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
        test_vin.c_str(),
        expires_at,
        ad_buffer_with_flags,
        &ad_length_with_flags,
        1, // flags = 1
        nullptr, 0
    );
    ...
    while (pos < ad_length_no_flags) {
        ...
        if (tag == 0xFF) break; // Terminal byte
...
TEST_F(ADBufferConstructionTest, DifferentDomainsProduceDifferentBuffers) {
    ...
    int result2 = infotainment_peer->constructADBuffer(
        Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
        test_vin.c_str(),
        expires_at,
        infotainment_buffer,
    ...
    EXPECT_NE(memcmp(vcsec_buffer, infotainment_buffer, 
                     std::min(vcsec_length, infotainment_length)), 0);
}
...
TEST_F(ADBufferConstructionTest, InvalidParametersReturnError) {
    ...
    // Test null VIN
    int result3 = vcsec_peer->constructADBuffer(
        Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
        nullptr,
        2655,
        ad_buffer,
...
TEST_F(ADBufferConstructionTest, RequestHashDomainDifferences) {
    ...
    int result1 = vcsec_peer->constructRequestHash(
        Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
        long_tag, 32,
    ...
    int result2 = infotainment_peer->constructRequestHash(
        Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
        long_tag, 32,
        infotainment_hash, &infotainment_hash_length
    );
