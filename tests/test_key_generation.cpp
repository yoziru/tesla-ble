...
TEST_F(KeyGenerationTest, CreatePrivateKeyGeneratesValidKey) {
  ...
  int get_status = client->get_private_key(private_key_buffer, sizeof(private_key_buffer), &private_key_length);
...
TEST_F(KeyGenerationTest, LoadEmptyPrivateKeyFails) {
  int status = client->load_private_key(nullptr, 0);
...
TEST_F(KeyGenerationTest, GetPrivateKeyWithoutLoadingFails) {
  ...
  size_t private_key_length;
  int status = client->get_private_key(private_key_buffer, sizeof(private_key_buffer), &private_key_length);
...
TEST_F(KeyGenerationTest, GetPrivateKeyWithInsufficientBufferFails) {
  ...
  size_t private_key_length;
  int get_status = client->get_private_key(small_buffer, sizeof(small_buffer), &private_key_length);
