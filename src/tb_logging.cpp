                     ...
                     const UniversalMessage_Destination *dest)
{
    ...
    case UniversalMessage_Destination_routing_address_tag:
        LOG_DEBUG("  routing_address: %s", format_hex(dest->sub_destination.routing_address.bytes, dest->sub_destination.routing_address.size).c_str());
        break;
    default:
...
void log_session_info(const char *tag, const Signatures_SessionInfo *req)
{
    ...
    LOG_DEBUG("  status: %s", req->status == Signatures_Session_Info_Status_SESSION_INFO_STATUS_OK ? "OK" : "KEY_NOT_ON_WHITELIST");
}
...
void log_signature_data(const char *tag, const Signatures_SignatureData *sig)
{
    ...
    if (sig->has_signer_identity)
    {
        ...
        LOG_DEBUG("      public_key: %s", format_hex(sig->signer_identity.identity_type.public_key.bytes, sig->signer_identity.identity_type.public_key.size).c_str());
    }
    ...
    case Signatures_SignatureData_session_info_tag_tag:
        LOG_DEBUG("    session_info_tag: %s", format_hex(sig->sig_type.session_info_tag.tag.bytes, sig->sig_type.session_info_tag.tag.size).c_str());
        break;
...
void log_information_request(const char *tag, const VCSEC_InformationRequest *msg)
{
    ...
    LOG_DEBUG("  publicKeySHA1: %s", format_hex(msg->key.keyId.publicKeySHA1.bytes, msg->key.keyId.publicKeySHA1.size).c_str());
...
void log_routable_message(const char *tag, const UniversalMessage_RoutableMessage *msg)
{
    ...
    case UniversalMessage_RoutableMessage_protobuf_message_as_bytes_tag:
        ...
        LOG_DEBUG("    payload: %s", format_hex(msg->payload.protobuf_message_as_bytes.bytes, msg->payload.protobuf_message_as_bytes.size).c_str());
        break;
...
void log_vssec_signed_message_status(const char *tag, const VCSEC_SignedMessage_status *status)
{
    ...
    LOG_INFO("    signed_message_information: %s", vssec_signed_message_information_to_string(status->signedMessageInformation));
}
...
void log_vssec_whitelist_operation_status(const char *tag, const VCSEC_WhitelistOperation_status *status)
{
    ...
    if (status->has_signerOfOperation)
    {
        ...
        LOG_DEBUG("      public_key: %s", format_hex(status->signerOfOperation.publicKeySHA1.bytes, status->signerOfOperation.publicKeySHA1.size).c_str());
    }
    LOG_INFO("    operation_status: %s", vcsec_operation_status_to_string(status->operationStatus));
    LOG_INFO("    information: %s", vssec_whitelist_operation_information_to_string(status->whitelistOperationInformation));
}
...
void logChargingState(const char *tag, const CarServer_ChargeState_ChargingState& state) {
    ...
    LOG_DEBUG("Charging State: %s", stateStr);
}
