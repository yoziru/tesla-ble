 ...
 */
void Client::prependLength(const pb_byte_t *input_buffer, size_t input_buffer_length, pb_byte_t *output_buffer,
 ...
 */
int Client::buildWhiteListMessage(Keys_Role role, VCSEC_KeyFormFactor form_factor, pb_byte_t *output_buffer,
  ...
  if (role < _Keys_Role_MIN || role > _Keys_Role_MAX) {
    LOG_ERROR("[buildWhiteListMessage] Invalid role value: %d (valid range: %d-%d)", role, _Keys_Role_MIN,
              _Keys_Role_MAX);
    return TeslaBLE_Status_E_ERROR_INVALID_PARAMS;
  }
  ...
  TeslaBLE::Client::prependLength(vcsec_encode_buffer, vcsec_encode_buffer_size, output_buffer, output_length);
  return TeslaBLE_Status_E_OK;
}
...
int Client::parseUniversalMessage(pb_byte_t *input_buffer, size_t input_size,
...
int Client::buildUniversalMessageWithPayload(pb_byte_t *payload, size_t payload_length, UniversalMessage_Domain domain,
                                             pb_byte_t *output_buffer, size_t *output_length, bool encrypt_payload) {
  LOG_DEBUG("[buildUniversalMessageWithPayload] Called with payload=%p, payload_length=%zu, domain=%d", payload,
            payload_length, domain);

  // Reject empty or null payloads
  if (payload == nullptr || payload_length == 0) {
    LOG_ERROR("[buildUniversalMessageWithPayload] Payload is null or empty (payload=%p, length=%zu)", payload,
              payload_length);
    return TeslaBLE_Status_E_ERROR_INVALID_PARAMS;
  }
  ...
  if (encrypt_payload) {
    ...
    session->constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED, vin_.c_str(), expires_at,
    ...
    return_code = session->constructRequestHash(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
 ...
 * @return int result code 0 for successful
 */
int Client::buildSessionInfoRequestMessage(UniversalMessage_Domain domain, pb_byte_t *output_buffer,
                                           size_t *output_length) {
 ...
 * @return int result code 0 for successful
 */
int Client::buildUnsignedMessagePayload(VCSEC_UnsignedMessage *message, pb_byte_t *output_buffer, size_t *output_length,
  ...
  return this->buildUniversalMessageWithPayload(payload_buffer, payload_length,
                                                UniversalMessage_Domain_DOMAIN_VEHICLE_SECURITY, output_buffer,
                                                output_length, encrypt_payload);
}

int Client::buildKeySummary(pb_byte_t *output_buffer, size_t *output_length) {
...
int Client::buildCarServerActionPayload(CarServer_Action *action, pb_byte_t *output_buffer, size_t *output_length) {
...
int Client::buildCarServerGetVehicleDataMessage(pb_byte_t *output_buffer, size_t *output_length,
...
int Client::buildCarServerVehicleActionMessage(pb_byte_t *output_buffer, size_t *output_length,
...
int Client::setCabinOverheatProtection(pb_byte_t *output_buffer, size_t *output_length, bool on, bool fan_only) {
...
int Client::scheduleSoftwareUpdate(pb_byte_t *output_buffer, size_t *output_length, int32_t offset_sec) {
  return this->buildCarServerVehicleActionMessage(
      output_buffer, output_length, CarServer_VehicleAction_vehicleControlScheduleSoftwareUpdateAction_tag,
      &offset_sec);
}

int Client::cancelSoftwareUpdate(pb_byte_t *output_buffer, size_t *output_length) {
  return this->buildCarServerVehicleActionMessage(
      output_buffer, output_length, CarServer_VehicleAction_vehicleControlCancelSoftwareUpdateAction_tag, nullptr);
}

int Client::buildVCSECActionMessage(const VCSEC_RKEAction_E action, pb_byte_t *output_buffer, size_t *output_length) {
...
int Client::buildVCSECInformationRequestMessage(VCSEC_InformationRequestType request_type, pb_byte_t *output_buffer,
...
int Client::buildVCSECClosureMessage(const VCSEC_ClosureMoveRequest *closure_request, pb_byte_t *output_buffer,
                                     size_t *output_length) {
  if (closure_request == nullptr || output_buffer == nullptr || output_length == nullptr) {
    LOG_ERROR("[buildVCSECClosureMessage] Invalid parameters");
    return TeslaBLE_Status_E_ERROR_INVALID_PARAMS;
  }
