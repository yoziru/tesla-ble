...
TEST_F(KeyGenerationTest, LoadInvalidPrivateKeyFails) {
  ...
  int status =
      client->loadPrivateKey(reinterpret_cast<const unsigned char *>(malformed_pem), strlen(malformed_pem) + 1);
...
-----END RSA PRIVATE KEY-----)";
  status = client->loadPrivateKey(reinterpret_cast<const unsigned char *>(rsa_key), strlen(rsa_key) + 1);
  ...
  status = client->loadPrivateKey(reinterpret_cast<const unsigned char *>(no_header), strlen(no_header) + 1);
...
TEST_F(KeyGenerationTest, GetPrivateKeyWithInsufficientBufferFails) {
  ...
  int load_status =
      client->loadPrivateKey(reinterpret_cast<const unsigned char *>(TestConstants::CLIENT_PRIVATE_KEY_PEM),
                             strlen(TestConstants::CLIENT_PRIVATE_KEY_PEM) + 1);
