...
TEST_F(ProtocolAuthenticationTest, CounterEdgeCases) {
  ...
  for (int i = 0; i < 10; ++i) {
    uint32_t counter_before = peer.get_counter();
    peer.increment_counter();
    uint32_t counter_after = peer.get_counter();
...
TEST_F(ProtocolAuthenticationTest, NonceGeneration) {
  ...
  peer.generate_nonce(nonce1);
  peer.generate_nonce(nonce2);
  peer.generate_nonce(nonce3);
...
TEST_F(ProtocolAuthenticationTest, SessionStateValidation) {
  ...
  EXPECT_FALSE(peer.has_valid_epoch()) << "Peer should not have valid epoch initially";
  ...
  EXPECT_TRUE(peer.has_valid_epoch()) << "Peer should have valid epoch after session update";

  // Verify session data was stored correctly
  EXPECT_EQ(peer.get_counter(), 50) << "Counter should match session info";
  EXPECT_EQ(memcmp(peer.get_epoch(), test_epoch, 16), 0) << "Epoch should match session info";
}
