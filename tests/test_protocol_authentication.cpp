...
TEST_F(ProtocolAuthenticationTest, MetadataSerializationEdgeCases) {
  ...
  int result = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
  ...
  result = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED, TestConstants::LONG_VIN,
                                  5000, ad_buffer, &ad_length);
  ASSERT_EQ(result, 0) << "Failed to construct AD buffer with max counter";

  // Test with flags set
  result = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED, TestConstants::LONG_VIN,
                                  6000, ad_buffer, &ad_length,
...
TEST_F(ProtocolAuthenticationTest, CounterEdgeCases) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (counter_before == UINT32_MAX) {
      ...
      EXPECT_TRUE(counter_after == 0 || counter_after == UINT32_MAX) << "Counter rollover should be handled gracefully";
    } else {
...
TEST_F(ProtocolAuthenticationTest, ErrorConditions) {
  ...
  result = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED, TestConstants::TEST_VIN,
                                  1000, ad_buffer, &ad_length);
  EXPECT_EQ(result, 0) << "AD buffer construction should work with default session values";

  // Test null pointer handling
  size_t dummy_length;
  result = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
                                  nullptr,  // null VIN
  ...
  result = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED, TestConstants::TEST_VIN,
                                  1000, small_buffer, &dummy_length);
...
TEST_F(ProtocolAuthenticationTest, ProtocolCompatibility) {
  ...
  // Test AES-GCM
  int result1 = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_AES_GCM_PERSONALIZED,
                                       TestConstants::TEST_VIN, 1000, ad_buffer1, &ad_length1);

  // Test HMAC
  int result2 = peer.constructADBuffer(Signatures_SignatureType_SIGNATURE_TYPE_HMAC_PERSONALIZED,
