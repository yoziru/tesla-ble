...
class VehicleActionBuilder {
 public:
  using BuilderFunction = TeslaBLEStatus (*)(CarServer_VehicleAction &, const void *);
 ...
 private:
  ...
  static TeslaBLEStatus build_vehicle_control_cancel_software_update(CarServer_VehicleAction &action, const void *data);
  static TeslaBLEStatus build_vehicle_control_reset_valet_pin(CarServer_VehicleAction &action, const void *data);
  static TeslaBLEStatus build_vehicle_control_reset_pin_to_drive(CarServer_VehicleAction &action, const void *data);
  static TeslaBLEStatus build_driving_clear_speed_limit_pin_admin(CarServer_VehicleAction &action, const void *data);
  static TeslaBLEStatus build_vehicle_control_reset_pin_to_drive_admin(CarServer_VehicleAction &action,
                                                                       const void *data);
  ...
  static TeslaBLEStatus build_vehicle_control_schedule_software_update(CarServer_VehicleAction &action,
                                                                       const void *data);
  ...
  // Helper functions
  static TeslaBLEStatus validate_input_parameters(const pb_byte_t *output_buffer, const size_t *output_length);
  static TeslaBLEStatus validate_charging_limit(int32_t percent);
  static TeslaBLEStatus validate_charging_amps(int32_t amps);
};
