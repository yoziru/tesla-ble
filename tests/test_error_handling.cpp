...
TEST_F(ErrorHandlingTest, BuildMessageWithoutPrivateKey) {
  ...
  int32_t amps = 12;
  int result1 = client->buildCarServerVehicleActionMessage(buffer, &length,
  ...
  int32_t percent = 80;
  int result2 = client->buildCarServerVehicleActionMessage(
  ...
  bool hvac_on = true;
  int result3 =
      client->buildCarServerVehicleActionMessage(buffer, &length, CarServer_VehicleAction_hvacAutoAction_tag, &hvac_on);
...
TEST_F(ErrorHandlingTest, BuildMessageWithNullParameters) {
  ...
  // Test with both null
  int result3 = client->buildCarServerVehicleActionMessage(nullptr, nullptr,
                                                           CarServer_VehicleAction_setChargingAmpsAction_tag, &amps);
...
TEST_F(ErrorHandlingTest, EdgeCaseFunctionCalls) {
  ...
  int32_t amps = 12;
  int result2 = client->buildCarServerVehicleActionMessage(small_buffer, &small_length,
                                                           CarServer_VehicleAction_setChargingAmpsAction_tag, &amps);
  EXPECT_NE(result2, 0) << "Building message with insufficient buffer should fail";

  // Test with boundary charge amounts
  int32_t zero_amps = 0;
  int result3 = client->buildCarServerVehicleActionMessage(
  ...
  int result4 = client->buildCarServerVehicleActionMessage(
  ...
  int result5 = client->buildUniversalMessageWithPayload(empty_payload, 0, UniversalMessage_Domain_DOMAIN_INFOTAINMENT,
